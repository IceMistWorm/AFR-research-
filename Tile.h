// This is the code written by Abhinav Srinivasan on his master's thesis, which can be found on: https://github.com/asriniv7/Adaptive-Frameless-Rendering
...
// Cite : modification heap code from https://www.geeksforgeeks.org/binary-heap/
...
class MinHeap
{
	...
	int *parent_index;// pointer to the array index mapping to parent tile nodes
...
public:
	...
	// Inserts a new key 'k' 
	void insertKey(int parent, float k);
};
...
class MaxHeap
{
	...
	int *leaf_index;// pointer to the array index mapping to parent tile nodes
...
public:
	...
	// Inserts a new key 'k' 
	void insertKey(int leaf, float k);
};
...
MaxHeap::MaxHeap(int cap)
{
	...
	capacity = cap;
	harr = new float[cap];
	leaf_index = new int[cap];
}

// Inserts a new key 'k' 
void MinHeap::insertKey(int p, float k)
{
	...
	while (i != 0 && harr[parent(i)] > harr[i])
	{
		...
		swap_index(&parent_index[i], &parent_index[parent(i)]);
		i = parent(i);
	}
}

// Inserts a new key 'k' 
void MaxHeap::insertKey(int p, float k)
{
	if (heap_size == capacity)
	{
		std::cout << "\nOverflow: Could not insertKey\n";
	...
	{
		swap(&harr[i], &harr[parent(i)]);
		swap_index(&leaf_index[i], &leaf_index[parent(i)]);
		i = parent(i);
	}
}
...
int MinHeap::extractMin()
{
	...
	// Store the parent_index of minimum value , and remove it from heap 
	//int root = harr[0];
	int root = parent_index[0];
	harr[0] = harr[heap_size - 1];
	parent_index[0] = parent_index[heap_size - 1];
	heap_size--;
	MinHeapify(0);
...
int MaxHeap::extractMax()
{
	...
	// Store the leaf_index of minimum value , and remove it from heap 
	//int root = harr[0];
	int root = leaf_index[0];
	harr[0] = harr[heap_size - 1];
	leaf_index[0] = leaf_index[heap_size - 1];
	heap_size--;
	MaxHeapify(0);
...
// This function deletes key at index i. It first increase value to max 
...
void MinHeap::MinHeapify(int i)
{
	...
	if (smallest != i)
	{
		...
		swap_index(&parent_index[i], &parent_index[smallest]);
		MinHeapify(smallest);
	}
}
...
void MaxHeap::MaxHeapify(int i)
{
	...
	if (biggest != i)
	{
		...
		swap_index(&leaf_index[i], &leaf_index[biggest]);
		MaxHeapify(biggest);
	}
}
